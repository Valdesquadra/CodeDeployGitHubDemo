...
if ( ! class_exists( 'mb2builderApi' ) )
{
	...
	class mb2builderApi
	{
		 ...
		 */
		 ...
		 /*
		  ...
		  public static function get_builder_settings( $opts = array() )
		  {
			  ...
			  $output .= format_text('[mb2pb_row template="1"][/mb2pb_row]');
			  ...
			  $output .= self::modal_template('elements');
		  ...
		  public static function export_page( $data, $filename )
		  {
			 ...
			 // $data = Mb2builderPagesApi::get_record( 7 );
			 // $data = $data->democontent;
		...
		public static function modal_template( $type = '', $opts = array() )
		{
			...
			$output .= $type === 'elements' ? self::get_layout_elements() : '';
		...
		public static function import_export( $opts = array() )
		{
			...
			$output .= '<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#tab-importrows">' . get_string('importrows','local_mb2builder') . '</a></li>';
			...
			$output .= '<div id="tab-importrows" class="tab-pane fade">';
			...
			//$output .= '<textarea id="mb2-pb-import-json" class="form-control mb2-pb-mb2color mb2-pb-input"></textarea>';
			//$output .= '<a href="#" id="mb2-pb-import-btn">' . get_string('import','local_mb2builder') . '</a>';
			//$output .= '</div>';
			$output .= self::get_latout_form();
			$output .= '</div>';

			$output .= '<div id="tab-export" class="tab-pane fade">';
			//$output .= '<a href="' . self::export_page( $opts['pagedata'], $opts['filename'] ) . '" class="btn btn-success" download>' .	get_string( 'downloadcontent','local_mb2builder' ) . '</a>';
		...
		public static function get_import_blocks_list( $layout = false )
		{
			...
			$output .= '<span>' . get_string( 'category', 'local_mb2builder' ) . '</span> <select name="mb2pb_select_block" id="mb2pb_select_block">';
