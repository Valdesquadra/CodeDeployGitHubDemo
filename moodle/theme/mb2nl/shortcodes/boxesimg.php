...
function mb2_shortcode_boxesimg_item( $atts, $content = null ){
	...
	$output .= $link !== '' ? '<a class="theme-boximg-img-link" href="' . $link . '"' . $target . '>' : '';
	...
	if ( $GLOBALS['boxlinkbtn'] )
	{
		...
		{
			$output .= '<a href="' . $link . '"' . $target . ' class="btn btn-' . $GLOBALS['boxlinkbtntype'] . ' btn-' . $GLOBALS['boxlinkbtnsize'] . ' rounded' . $GLOBALS['boxlinkbtnrounded'] . ' btnborder' . $GLOBALS['boxlinkbtnborder'] . '">' . $btntext . '</a>';
		}
		else
		{
			$output .= '<a class="arrowlink" href="' . $link . '"' . $target . '>' . $btntext . '</a>';
		}
