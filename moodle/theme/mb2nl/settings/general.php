...
$temp->add($setting);
	...
	$setting = new admin_setting_configmb2color($name, $title, '', '#0E314F');
	$setting->set_updatedcallback('theme_reset_all_caches');
	$temp->add($setting);
...
$temp->add($setting);
	...
	$setting = new admin_setting_configtextarea($name, $title, '', 'Copyright (c) New Learning Theme ' . date('Y') . '. All rights reserved.');
	$temp->add($setting);
