 ...
 *
 */
function theme_mb2nl_course_list_courses( $courses, $box = false, $builder = false )
{
	global $CFG, $PAGE;
	$output = '';
	...
	foreach ( $courses as $course )
	{
		...
		$output .= '<div class="theme-course-item course-' . $course->id . $cls . $catcls . $bestcls . $pricecls . '" data-course="' . $course->id . '">';
...
function theme_mb2nl_coursetabs_tabs( $opts )
{
	...
	$output .= '<ul class="coursetabs-tablist">';
	...
	foreach( $categories as $category )
	{
		...
		$output .= '<li class="coursetabs-catitem' . $isactive . '" data-category="' . $category->id . '">';
		$output .= '<span class="catname">' . $category->name . '</span>';
		$output .= '<span class="coursecount">(' . $coursecount . ')</span>';
		$output .= '</li>'; // coursetabs-catitem
	}

	$output .= '</ul>'; // coursetabs-tablist
...
function theme_mb2nl_coursetabs_courses( $opts )
{
	...
	foreach( $categories as $category )
	{
		...
		$output .= '<div class="coursetabs-content' . $fillcls . '" id="category-content-' . $category->id  . '">';
...
function theme_mb2nl_coursetabs_tabcontent( $opts )
{
	...
	$list_cls = '';
	$list_cls .= $opts['carousel'] ? ' owl-carousel' : '';
 ...
 *
 */
function theme_mb2nl_category_image( $catid )
{
	global $CFG;
	require_once( $CFG->libdir . '/filelib.php' );
