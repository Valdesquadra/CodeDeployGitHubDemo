 ...
 *
 */
function theme_mb2nl_get_section_activities( $sectionid = 0 )
{
 ...
 *
 */
function theme_mb2nl_section_module_list( $sectionid, $link = false, $active = false )
{
	global $PAGE;
	$output = '';
	$modules = theme_mb2nl_get_section_activities( $sectionid );
	...
	foreach ( $modules as $m )
	{
		...
		$modactive = $active && $PAGE->cm->id == $m['id'] ? ' active' : '';
		$modcomplete = theme_mb2nl_module_complete($m['id']) ? ' complete' . theme_mb2nl_module_complete($m['id']) : '';

		$output .= '<li class="module-' . $m['modname'] . $modactive . $modcomplete . '">';
		$output .= $link ? '<a href="' . $modlink . '">' : '';
		$output .= '<span class="itemimage"><img src="' . $m['icon'] . '" alt="' . $m['name'] . '"></span>';
		$output .= '<span class="itemname">' . $m['name'] . '</span>';
		$output .= $link ? '</a>' : '';
		$output .= '</li>';
	}
...
function theme_mb2nl_activities_list( $ccourse = false )
{
	...
	{
		$output .= '<li><img src="' . $a['icon'] . '" alt="' . $a['title'] . '" />' . $a['title'] . '</li>';
	}
	$output .= '</ul>';
...
function theme_mb2nl_get_enroll_hero_url()
{
	...
	return $OUTPUT->image_url( 'enrol-hero-default','theme');
...
function theme_mb2nl_course_sections_accordion()
{
	...
	foreach ( $sections as $section )
	{
		...
		$output .= '<h5>';
		$output .= '<span data-toggle="collapse" data-target="#' . $collid. '" aria-expanded="' . $isexpand . '" aria-controls="' . $collid. '">';
		$output .= $section['name'];
		$output .= '</span>';
		$output .= '</h5>';
		$output .= '</div>';
		...
		$output .= theme_mb2nl_section_module_list( $section['id'] );
	    $output .= '</div>';
	    $output .= '</div>';
 ...
 *
 */
function theme_mb2nl_get_course_sections( $ccourse = false )
{
	global $COURSE;
...
function theme_mb2nl_get_category_courses_count( $catid, $visible = false )
{
	...
	if ( $visible )
	{
		$andcourses = '  AND c.visible = :visible';
	}
	...
	$sqlquery = 'SELECT COUNT(c.id) FROM {course} c WHERE c.category = :catid' . $andcourses . $anddate;
...
function theme_mb2nl_get_user_description( $description, $userid )
{
	...
	$desc = file_rewrite_pluginfile_urls( $description, 'pluginfile.php', $usercontext->id, 'user', 'description', NULL );
	$desc = format_text( $desc, FORMAT_HTML );
...
function theme_mb2nl_get_course_video()
{
	...
	$output = '';
	$settings = theme_mb2nl_enrolment_options();
	$videofile = theme_mb2nl_get_format_video_url();

	if ( ! $settings->introvideourl && ! $videofile )
	{
		return;
	}
	...
	if ( $videofile )
	{
		$output .= '<video controls="true" title="This is video title" width="1900">';
 ...
 *
 */
function theme_mb2nl_get_format_video_url()
{
	global $CFG, $COURSE;
...
function theme_mb2nl_get_featured_reviews( $opts = array() )
{
	...
	$params = array();
	$sqlwhere = ' WHERE 1=1';
	$sqlquery = 'SELECT * FROM {local_mb2reviews_items}';

	$sqlwhere .= ' AND enable=1';
	$sqlwhere .= ' AND featured=1';
...
function theme_mb2nl_full_screen_module()
{
	...
	if ( $PAGE->user_is_editing() || theme_mb2nl_has_builderpage() || $COURSE->id <= 1 )
	{
		return false;
	}
 ...
 *
 */
function theme_mb2nl_full_screen_module_sections()
{
	$output = '';
	$sections = theme_mb2nl_get_course_sections();

	$output .= '<div class="fsmod-sectionlist">';
	...
	foreach ( $sections as $section )
	{
		...
		$output .= '<div class="fsmod-section fsmod-section-' . $section['num'] . $completecls . '">';
		$output .= '<div class="fsmod-section-tite">';
		$output .= '<span class="title-text">' . $section['name'] . '</span>';
		$output .= $completepercentage !== false ? '<span class="title-complete">(' . $completepercentage . '%)</span>' : '';
		$output .= '</div>'; //fsmod-section-tite
		$output .= '<div class="fsmod-section-modules">';
		$output .= theme_mb2nl_section_module_list( $section['id'], true, true );
		$output .= '</div>'; //fsmod-section-modules
		$output .= '</div>'; //fsmod-section
	}

	$output .= '</div>'; // fsmod-sectionlist
...
function theme_mb2nl_full_screen_module_backlink()
{
	...
	$linkstr = get_string('backto', 'core', $COURSE->fullname );

	$output .= '<a href="' . $backtocourseurl . '" class="fsmod-backlink" data-toggle="tooltip" title="' . $linkstr . '">';
...
/*
 *
 * Method to get course activity navigation
 *
 */
function theme_mb2nl_activity_nav()
{
	...
	$output .= '<div class="fsmod-activity-nav">';

	if ( $prevmod )
	{
		$prevlink = new moodle_url( $prevmod['url'], array('forceview'=>1) );
		$output .= '<a href="' . $prevlink . '" class="nav-prev" data-toggle="tooltip" title="' . $prevmod['name'] . '">';
	...
	if ( $nextmod )
	{
		...
		$output .= '<a href="' . $nextlink . '" class="nav-next" data-toggle="tooltip" title="' . $nextmod['name'] . '">';
		$output .= '<i class="fa fa-angle-right"></i>';
		$output .= '</a>'; // nav-next
	}

	$output .= '</div>'; // fsmod-activity-nav
