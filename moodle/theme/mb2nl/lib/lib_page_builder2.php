...
function theme_mb2nl_page_builder_el_actions( $element = 'element', $elstr = '', $opts = array() )
{
	...
	. '" data-toggle="modal" data-target="#mb2-pb-modal-settings-' . $element . '">';
...
function theme_mb2nl_page_builder_el_datatts( $atts, $atts2 = array() )
{
	...
	foreach ( $atts2 as $k => $v )
	{
		...
		// We need use 'htmlentities' because this will in html data attribute
		// This menas that value contains html tags
		// So, we have to remove these tags
		$isquote = ( strpos( $v, '"') !== false && strpos( $v, '"') >= 0 );
		if ( strip_tags( $v ) !== $v || $isquote  )
		{
			$v1 = htmlentities( $v );
		}

		// In this case value doesn't contain html tags so we don't want to remove it
		// this is important because if we use second time htmlentities shortcode will output html etities (&lt;p&gt;)
		else
		{
			$v1 = $v;
		}
 ...
 *
 */
function theme_mb2nl_page_builder_demo_image( $name = '600x400' )
{
	global $CFG, $OUTPUT;
...
function theme_mb2nl_page_builder_pagelink()
{
	...
	$output .= 'data-toggle="tooltip" title="' . get_string( 'buildepage', 'local_mb2builder' ) .  '">';
	$output .= '<i class="fa fa-magic"></i>';
	$output .= '</a>';

	if ( $PAGE->pagetype === 'site-index' )
	{
		$fpivon = 'fa fa-refresh';
		$fptext = $PAGE->user_is_editing() ? get_string('turneditingoff') : get_string('turneditingon');

		$output .= '<a class="buildpage-fp" href="' . new moodle_url( '/course/view.php',
		array( 'id'=>$COURSE->id,'sesskey'=>$USER->sesskey, 'edit'=> $PAGE->user_is_editing() ? 'off' : 'on' ) ) . '" ';
		$output .= 'data-toggle="tooltip" title="' . $fptext .  '">';
		$output .= '<i class="' . $fpivon . '"></i>';
		$output .= '</a>';
	}
	elseif ( $PAGE->pagetype === 'mod-page-view' )
	{
		$linkmoodle = new moodle_url( $CFG->wwwroot . '/course/modedit.php',array( 'update' => $PAGE->cm->id, 'return' => 1 ) );
		$output .= '<a class="buildpage-moodle" href="' . $linkmoodle . '" data-toggle="tooltip" title="' . get_string( 'buildepagemoodle', 'local_mb2builder' ).  '"><i class="fa fa-edit"></i></a>';
	}
