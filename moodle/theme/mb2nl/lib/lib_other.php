...
function theme_mb2nl_body_cls()
{
	...
	user_preference_allow_ajax_update('theme-usersidebar', PARAM_ALPHA);
	$usersidebar = get_user_preferences('theme-usersidebar', 'true');
	...
	if ( theme_mb2nl_theme_setting($PAGE,'sidebarbtn') == 2 || $usersidebar === 'false' )
	{
	...
	if ( $PAGE->pagetype === 'grade-report-grader-index' && $USER->gradeediting[$COURSE->id] )
	{
		$output[] = 'grading';
	}
...
function theme_mb2nl_is_login( $custom = false )
{
	...
	$customLoginPage = theme_mb2nl_theme_setting($PAGE, 'cloginpage');
...
function theme_mb2nl_theme_scripts ($page, $attribs = array())
{
	...
	// We need this script only on page builder editinig page
	// On other pages it's loaded via AMD
	// if ( preg_match( '@mb2builder@', $PAGE->pagetype ) )
	// {
	// 	$page->requires->js(theme_mb2nl_theme_dir() . '/mb2nl/assets/OwlCarousel/owl.carousel.min.js');
	// }

	//$page->requires->css(theme_mb2nl_theme_dir() . '/mb2nl/assets/slick/slick/slick.css');

	// Prallax effect
	$page->requires->js(theme_mb2nl_theme_dir() . '/mb2nl/assets/jarallax/jarallax.min.js');
	...
	// tilt.jquery
	//$page->requires->js(theme_mb2nl_theme_dir() . '/mb2nl/assets/tilt/tilt.jquery.js');
 ...
 *
 */
function theme_mb2nl_logo_url( $custom = false, $dark = false )
{
	...
	$logourl = '';
	$iscustom = '';
	$logoname = $dark ? 'logo-dark' : 'logo-default';
	...
	$logourl = $CFG->version >= $moodle33 ? $OUTPUT->image_url( $logoname,'theme') : $OUTPUT->pix_url( $logoname, 'theme' );
 ...
 *
 */
function theme_mb2nl_get_course_fields( $courseid )
{
...
function theme_mb2nl_quest_login()
{
	...
	if ( isloggedin() || $CFG->forcelogin || ! $CFG->autologinguests || ! theme_mb2nl_theme_setting( $PAGE, 'autologinguestsanypage' ) )
	{
		return;
	}
